...
// int EncPinBlock2Envelope_BCM(int sockFd, int is_to_byte8, char* acco, 
// 							 char* symmKeyID, char* modAndPadding, unsigned char* iv, int ivlen,
// 							 unsigned char* oldCryptoText, int oldCryptoLen,
// 							 char* certDN, char*encAlg,
// 							 unsigned char *newCryptoEnv, int *newEnvLen,
// 							 unsigned char* newCryptoText, int *newCryptoLen);
// 
// int Envelope2EncPinBlock_BCM(int sockFd, int is_to_byte8, char* acco, 
// 							 char* certDN,
// 							 unsigned char *oldCryptoEnv, int oldEnvLen,
// 							 unsigned char* oldCryptoText, int oldCryptoLen,
// 							 char* symmKeyID, char* modAndPadding, unsigned char* iv, int ivlen, 
// 							 unsigned char* newCryptoText, int *newCryptoLen);
int EncPin2EnvelopePinblock_BCM(int sockFd, char* acco, int is_to_byte8,
								char* symmKeyID, SymmEncryptParam *decAlg,
								unsigned char* oldCryptoText, int oldCryptoLen,
								char* certDN, SymmEncryptParam *encAlg,
								unsigned char *newCryptoEnv, int *newEnvLen,
								unsigned char* newCryptoText, int *newCryptoLen);

int EnvelopePinblock2EncPin_BCM(int sockFd, char* acco, 
								char* certDN, SymmEncryptParam *decAlg,
								unsigned char *oldCryptoEnv, int oldEnvLen,
								unsigned char* oldCryptoText, int oldCryptoLen,
								char* symmKeyID, SymmEncryptParam *encAlg, 
								unsigned char* newCryptoText, int *newCryptoLen);



int RawSign_GetPubKey_BCM(int sockFd, unsigned char* plain, int iPlainLen, char* signCertDN, char* digestAlg, 
					  unsigned char* crypto, int* iCryptoLen,
					  unsigned char* pubkey, int * iPubkey);

int MultiEnvEncrypt_BCM(int sockFd, char* certid, SymmEncryptParam *encAlg, 
						unsigned int arrayNum, DataLV inDatas[], 
						unsigned char* outBuff, unsigned buffMaxLen,
						DataLV *envCrypto, DataLV  outdatas[]);

int MultiEnvDecrypt_BCM(int sockFd, char* certid, SymmEncryptParam *decAlg, 
						unsigned int arrayNum, DataLV inDatas[], 
						DataLV *envCrypto,
						unsigned char* outBuff, unsigned buffMaxLen,
						DataLV  outDatas[]);

int GenSessionKey_BCM(int sockFd, char* certID, char * prefix, /*1233|01|*/ 
					  char* CryptoSkey);

int RewrapSessionKey_BCM(int sockFd, char *oldCertID, char * oldCryptoSkey, 
						 char * newPrefix, char * newCertID, char *newCryptoSkey);//base64

int EncryptFile_BCM(int sockFd, char *certID, char * CryptoSkey,SymmEncryptParam *encalg,  
					char *plain_file_path, char *cipher_file_path);

int DecryptFile_BCM(int sockFd, char *certID, char * CryptoSkey,SymmEncryptParam *decalg,
					char *cipher_file_path ,char *plain_file_path,char * prefix);

// int Encrypt_RD_BCM(int sockFd, char *plain_file_path, char *certid, char *bankid, char *alg, char *cipher_file_path, char *envlope_path);
// 
// 
// int Decrypt_RD_BCM(int sockFd, char *cipher_file_path, char *envlope_path, char *certid, char *plain_file_path);
...
typedef enum
{
	...
	SEN_TYPE_BCM_ENV_ENCRYPT,/*BCM  普通数字信封加解密多块数据*/
}ENUM_SEN_TYPE;
...
/* 3.	获取CRL中作废证书数量
	...
	int sockFd				in	签名服务器连接句柄
	char* trustField		in	信任域名称或者CA的主题
...
int INS_KPLCountCRL(int sockFd, char* trustField, unsigned int *crlCount);
...
{
	char bankid[64+1];					/*bankid或者 keyid*/
	char serialNumber[40+1];				/* 证书序列号*/
	char subject[512+1];					/* 证书主题*/
}KPLCertInfo;
